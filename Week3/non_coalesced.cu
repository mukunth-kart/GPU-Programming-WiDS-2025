...
__global__ void mult_scale(float* A, float* B, float* C, float scale, int n)
{
	...
	int stride = blockDim.x * gridDim.x; //In sample codes this was the stride, if I understand reason will put in accompanying pdf.
	//Bounds Checking
	for( int i = work_thread; i<n; i += stride ) 
	{
		C[i] = A[i]*B[i]*scale;
	}
}
